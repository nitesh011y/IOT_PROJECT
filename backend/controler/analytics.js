...
function Analytics() {
  ...
  const [sosPreceding, setSosPreceding] = useState(null);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        // Once timeline is loaded, compute descriptive stats
    ...
    axios
      .get("http://localhost:5000/api/analytics/sos-preceding?windowSec=10")
      .then((res) => setSosPreceding(res.data));
    axios
      .get("http://localhost:5000/api/analytics/heatmap")
      .then((res) => setHeatmap(res.data));
  }, []);

  // Re-run stats when overview, obstacleBreakdown, or timeline changes
  useEffect(() => {
  ...
  const computeDescriptiveStats = (
    ...
    // 2. Obstacle distance percentages
    const obstacleMap = {};
    ...
    // 3. Most common obstacle distance (mode)
    ...
    // 4. Calculate date range from timeline
    let minDate = null,
      maxDate = null;
    if (timelineData.length > 0) {
      // timeline _id is like "2026-04-03 11:00" – extract date part
    ...
    if (timelineData.length > 0) {
      timelineData.forEach((t) => {
        if (t.totalEvents > maxEventsHour) {
          ...
          mostActiveHour = t._id; // e.g., "2026-04-03 14:00"
        }
      });
    }
  ...
  return (
    <>
      ...
      <div className="analytics-wrapper">
        ...
        {/* ===== SOS PRECEDING INSIGHTS ===== */}
        {sosPreceding && sosPreceding.totalSOS > 0 && (
          <div className="sos-insights">
            <h3>
              SOS Preceding Events (within {sosPreceding.windowSeconds} seconds)
            </h3>
            <div className="sos-stats">
              <div className="stat">Total SOS: {sosPreceding.totalSOS}</div>
              <div className="stat">
                Water before SOS: {sosPreceding.precedingWater}
              </div>
              <div className="stat">
                VERY CLOSE obstacles before SOS:{" "}
                {sosPreceding.precedingVeryClose}
              </div>
              <div className="stat">
                Avg water per SOS: {sosPreceding.averagePerSOS?.water}
              </div>
              <div className="stat">
                Avg VERY CLOSE per SOS: {sosPreceding.averagePerSOS?.veryClose}
              </div>
            </div>
          </div>
        )}
        ...
        {heatmap.length > 0 && (
          <div className="heatmap-section">
            <h3>🚨 SOS Heatmap (When SOS is pressed – by hour & day)</h3>
              ...
              <table className="heatmap-table">
                ...
                <tbody>
                  {heatmap.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx}>
                        <td>{dayNames[item._id.day] || item._id.day}</td>
                        <td>{item._id.hour}:00</td>
                        <td className="sos-count">{item.sosCount}</td>
                      </tr>
                    );
                  })}
                </tbody>
